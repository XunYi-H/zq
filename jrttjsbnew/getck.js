...
if (process.env.COOKIESERVER) {
    ...
    COOKIESERVER = process.env.COOKIESERVER.split("&")[0].split("@");

    token = COOKIESERVER[0]
    server = COOKIESERVER[1]
...
async function getCKS(appid,tag=null) {
    return new Promise(resolve => {
        let params = "appid=" + appid;
        if(tag!==null) params = params + "&tag=" + tag;
        datas  = {
            url: server + '/admin/projects/' + token + "?" + params,
            headers: {'accept': 'application/json'},
        }
      $.get(datas, (err, resp, data) => {
        try {
            if (err) {
                console.log("错误")
                console.log(err)
                ...
                if (data) {
                    ...
                    if (data !== []){
                        data = data.map(item=>item.cookies);
                        console.log("账号获取成功，获取到"+data.length.toString()+"个账号")
