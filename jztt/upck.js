...
async function getCk() {
    ...
    if ($request['url']['indexOf']('index/benefit') > -1) {
        let _0x20aef4 = $request['headers']['token'];
        if (awyapp[_0x23050f]) {
            awyapp[_0x23050f]['token'] = _0x20aef4;
        } else {
            awyapp[_0x23050f] = {
                'token': _0x20aef4
            };
        ...
        try{            
            ...
            $['setdata'](JSON['stringify'](awyapp, null, 0x2), 'jzttapp');
        }
        $['msg']($['name'], '九章头条基础数据获取成功！🎉');
        //判断环境变量存在，不存在，则连接字符串
    }
}
