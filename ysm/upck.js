...
async function getCk() {
    ...
    if($request.url.indexOf(`yunonline/v1/task`) > -1) {
        ...
        resdata = await upck(JSON.stringify(ua),awyapp[i])
        try{            
            awyapp[i]['sqlid'] = resdata.data.id
        }catch{
            print(resdata)
        }finally{
            $['setdata'](JSON['stringify'](awyapp, null, 0x2), 'ysmwx');
        }
        $['msg']($['name'], '云扫码获取到刷新任务参数');
        return
    }
    if($request.url.indexOf(`connect/oauth2/authorize`) > -1) {
        let ck = $request.url.split('authorize?')[1]
        if (awyapp[i]) {
            awyapp[i]['ysmWxParam'] = ck;
        } else {
            awyapp[i] = {
                'ysmWxParam': ck
            };
    ...
    if($request.url.indexOf(`yunonline/v1/gold`) > -1) {
        let ck = $request.url.split('unionid=')[1].split('&')[0]
        if (awyapp[i]) {
            awyapp[i]['ysmUnionid'] = ck;
        } else {
            awyapp[i] = {
                'ysmUnionid': ck
            };
