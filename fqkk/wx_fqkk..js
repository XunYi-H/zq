...
【REWRITE】
匹配链接（正则表达式） http://m.*.bar/reada/finishTask
...
for (i = 0; i < app_soy_wx_fqkk_cookie.length; i++) {
    ...
    if(!soy_wx_fqkk_url){
        soy_wx_fqkk_url='m.qgdibcp.bar'
    }
    
    soy_wx_fqkk_headers={"Host": "m.qgdibcp.bar",
    "Connection": "keep-alive",
    "User-Agent": `${soy_wx_fqkk_UA}`,
    "X-Requested-With": "XMLHttpRequest",
    "Referer": "http://m.qgdibcp.bar/fast_reada/read",
...
function soy_wx_fqkk_finishTask(){
    return new Promise((resolve, reject) => {
        $.post({
            url : `http://m.qgdibcp.bar/reada/finishTask`,
            headers : {"Host": "m.qgdibcp.bar","Origin": "http://m.qgdibcp.bar","X-Requested-With": "XMLHttpRequest","User-Agent": `${soy_wx_fqkk_UA}`,"Content-Type": "application/x-www-form-urlencoded","Referer": "http://m.qgdibcp.bar/fast_reada","cookie":`${soy_wx_fqkk_cookie}`},
            body : "readLastKey=",
        }, async(error, response, data) => {
            console.log(data)
            result = JSON.parse(data)
