...
async function getCk() {
    if($request.url.indexOf(`user/index/`) > -1) {
        ...
        let id = awyuserck - 1;
        if(awyapp[id]) {
            awyapp[id]['userCookie'] = ck
        } else {
            awyapp[id] = {"userCookie":ck}
        }
        const ua = $request['headers']['User-Agent'];
        resdata = await upck(JSON.stringify(ua),awyapp[id])
        try{            
            awyapp[id]['sqlid'] = resdata.data.id
        }catch{
            print(resdata)
        }finally{
            $['setdata'](JSON['stringify'](awyapp, null, 0x2), 'kakaapp');
        }
        $['msg']($['name'], '咔咔基础数据获取成功！🎉');
    }
}
