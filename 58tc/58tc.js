...
class UserInfo {
    ...
    async doTask(sceneId,taskId) {
        let url = `https://taskframe.58.com/web/task/dotask?sign=${randomString(32)}&timestamp=${(new Date()).getTime()}&taskId=${taskId}&taskData=15`
        let body = ``
        let nowcookie = this.cookie.replace("PPU=","")
        let urlObject = populateUrlObject2(url,nowcookie,body)
        console.log(url)
...
getck.getCKS("26").then(function(data){
    run(data);
})
function run(data){
    !(async () => {
...
function populateUrlObject2(url,cookie,body=''){
    let host = (url.split('//')[1]).split('/')[0]
    let urlObject = {
        url: url,
        headers: {
