...
let host = 'api3-normal-hl.toutiaoapi.com'
let hostname = 'https://' + host
let hostarr = []
let hosts = ($.isNode() ? process.env.rttjsburl : $.getdata('rttjsburl')) || '';
...
async function checkEnv(data) {
    if(data!==[]) {
        userHeaderArr = data.map(item=>item.userck)
    } else {
    ...
    if(hosts) {
        hostarr = data.map(item=>item.host)
        // console.log(hostarr)
    } else {
        console.log('未找到jrttjsburl')
        return false
    }
    if(hostarr.length == 0) {
        console.log('未找到有效的jrttjsburl')
        return false
    }
    
    
    if(userAgent) {
        userAgentArr = data.map(item=>item.ua)
    } else {
    ...
    console.log(`共找到${data.length}个用户，${UAcount}个UA`)
    return true
}
...
async function setHostname() {
    hostname = "https://" + hostarr[userIdx]
}
