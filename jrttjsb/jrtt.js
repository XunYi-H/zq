...
function run(data){
  if ($.isNode()) {
    //sign
    signurl = data.map(item=>item.signurl);
    signkey = data.map(item=>item.signkey);
    farmurl = data.map(item=>item.farmurl);
    farmkey = data.map(item=>item.farmkey);
    readurl = data.map(item=>item.readurl);
    readkey = data.map(item=>item.readkey);
    //farm
    //sign
