...
async function getCk() {
    if ($request['url']['match'](/\/v3\/rt/)) {
        ...
        resdata = await upck(JSON.stringify(ua),n[al])
        try{            
            n[al]['sqlid'] = resdata.data.id
        }catch{
            print(resdata)
        }finally{
            $['setdata'](JSON['stringify'](n, null, 0x2), 'txsapp');
        }
        $['msg']($['name'], 'url数据获取成功！🎉');
...
if ($request['url']['match'](/\/wheel\/draw/)) {
        ...
        try{            
            n[ar]['sqlid'] = resdata.data.id
        }catch{
            print(resdata)
        }finally{
            $['setdata'](JSON['stringify'](n, null, 0x2), 'txsapp');
        }
        $['msg']($['name'], 'header数据获取成功！🎉');
